...
This approach uses the Mac-only [Reality Converter](https://developer.apple.com/augmented-reality/tools/) utility to create a `.usdz` file for Procreate to use - there are almost certainly alternatives, but this is the method I got to work.
...
It sounds counterintuitive, but the best way[^2] to export to a format that Procreate will handle is to go via `.obj` first. This will ensure the texture coordinates don't get messed up[^3].
...
I've tried quite a few approaches to this, but the most reliable I found[^2] was to use ImageMagick, and I wrapped it up in a pair of shell scripts I collectively called `matknife`;
...
[^2]: If you know a better way to handle these, please let me know!

[^3]: Blender's `.usd` export option doesn't work properly for including textures; 2.9 exports in a way that is missing textures, and 3.0 exports with textures but their UV positions are incorrect. There's also a third-party plugin for direct `.usdz` export, but it crashes in my experience.
